...
fn middleware_runs_for_stream_route() {
    common::test_runtime()
        ...
        .run(|| {
            ...
            let response =
                crate::http::request(addr, "GET", "/download", &[], &[], Duration::from_secs(5))
                    .unwrap();
            assert_eq!(response.status, 200);
...
fn middleware_with_handler_responds() {
    common::test_runtime()
        .run(|| {
            ...
            let addr = common::spawn_server(router);
            let response =
                crate::http::request(addr, "GET", "/check", &[], &[], Duration::from_secs(5))
                    .unwrap();

            assert_eq!(response.status, 200);
            assert_eq!(response.header("x-mw-check"), Some("ran"));
