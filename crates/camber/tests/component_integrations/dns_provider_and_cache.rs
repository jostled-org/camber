...
mod cloudflare_provider_simulation {
    ...
    async fn cloudflare_auth_failure_returns_error() {
        ...
        assert!(
            ...
            "error should describe authentication failure: {message}"
        );
    }
}
...
mod certificate_cache {
    ...
    fn write_test_cache(cache_dir: &Path, days_until_expiry: i64) {
        let certified_key = rcgen::generate_simple_self_signed(vec!["test.example.com".into()])
            .expect("generate cert");

        std::fs::create_dir_all(cache_dir).expect("create cache dir");
        std::fs::write(cache_dir.join("cert.pem"), certified_key.cert.pem()).expect("write cert");
        std::fs::write(
