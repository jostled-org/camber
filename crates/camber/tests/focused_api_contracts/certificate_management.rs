...
fn cert_store_resolves_cert() {
    ...
    let key = tls_support::certified_key_from_pem(&cert_pem, &key_pem);
...
fn cert_store_hot_swap() {
    let (cert_a_pem, key_a_pem) = tls_support::generate_self_signed_cert();
    let (cert_b_pem, key_b_pem) = tls_support::generate_self_signed_cert();

    let key_a = tls_support::certified_key_from_pem(&cert_a_pem, &key_a_pem);
    let key_b = tls_support::certified_key_from_pem(&cert_b_pem, &key_b_pem);
...
fn resolve_tls_partial_returns_error() {
    let (cert_pem, key_pem) = tls_support::generate_self_signed_cert();
    let tmp = tempfile::tempdir().unwrap();
    let (cert_path, _key_path) = temp_support::write_cert_files(&tmp, &cert_pem, &key_pem);
