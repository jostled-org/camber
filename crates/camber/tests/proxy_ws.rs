...
{
    lifecycle_event("proxied WebSocket event before owner completion", async {
        tokio::select! {
            biased;
            result = owner.as_mut() => {
                panic!("owner completed before proxied WebSocket transport event: {result:?}")
            }
...
where
    F: Future<Output = Result<(), RuntimeError>>,
{
    lifecycle_event("transport EOF before proxy owner completion", async {
        let mut byte = [0u8; 1];
        tokio::select! {
            biased;
            result = owner.as_mut() => {
                panic!("proxy owner completed before transport EOF: {result:?}")
            }
...
async fn pending_proxy_upgrade_shutdown_is_rejected(forced: bool) {
    ...
    let mut owner = Box::pin(handle.into_future());
    let response = lifecycle_event("pending proxy rejection before owner completion", async {
        tokio::select! {
            biased;
            result = owner.as_mut() => {
                panic!("proxy owner completed before writing upgrade rejection: {result:?}")
            }
    ...
    assert!(
        ...
        "shutdown committed a proxy 101 response"
    );
    assert_eq!(
...
async fn supervisor_unwind_joins_acknowledged_and_pending_proxy_upgrades() {
    ...
    let pending_response = lifecycle_event(
        "pending proxy unwind response before owner completion",
        async {
            tokio::select! {
                biased;
                result = owner.as_mut() => {
                    panic!("proxy owner completed before pending wrapper terminated: {result:?}")
                }
                response = read_async_http_head(&mut pending) => response,
            }
        },
